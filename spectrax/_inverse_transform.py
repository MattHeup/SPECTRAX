...
from orthax.hermite import hermval, hermvander
...
def generate_Hermite_basis(N, x):
    ...
    """
    return jnp.moveaxis(hermvander(x, N - 1), -1, 0)
...
def generate_Hermite_term(C, Herm_x, Herm_y, Herm_z, Nn, Nm, Np, xi_x, xi_y, xi_z):
    ...
    """
    Nt, N_modes, Ny, Nx, Nz = C.shape
    _, _, Nvy, Nvx, Nvz = Herm_x.shape
    ...
    basis = (Hx * Hy * Hz) / norm[:, None, None, None, None]

    # C:     (Nt, N_modes, Ny, Nx, Nz)       -> tmyxz
    # basis: (N_modes, Nt, Nvy, Nvx, Nvz)    -> mtuvw
    # -> f:  (Nt, Ny, Nx, Nz, Nvy, Nvx, Nvz) -> tyxzuvw
    f = jnp.einsum('tmyxz,mtuvw->tyxzuvw', C, basis)

    gauss = jnp.exp(-(xi_x**2 + xi_y**2 + xi_z**2))
    return f * gauss[:, None, None, None, :, :, :]
...
def inverse_HF_transform(Ck, Nn, Nm, Np, xi_x, xi_y, xi_z):
    ...
    Herm_x = generate_Hermite_basis(Nn, xi_x)  # (Nt, Nn, Nvy, Nvx, Nvz)
    Herm_y = generate_Hermite_basis(Nm, xi_y)  # (Nt, Nm, Nvy, Nvx, Nvz)
    Herm_z = generate_Hermite_basis(Np, xi_z)  # (Nt, Np, Nvy, Nvx, Nvz)
